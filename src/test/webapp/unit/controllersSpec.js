...
describe('Social controllers', function(){
    ...
    beforeEach(inject(function($rootScope, $controller, LAHelper) {
      ...
      LAMock =  {
        ...
        getChirpGraph : function() {
          return {
            selection : function() { return ["china", "barry"]},
            each: function(filter, callback) { for(var i = 0; i < nodes.length; i++) callback({'id': nodes[i]}); },
            combo: function() { return {isCombo: function(){return false;}}},
            zoom : function() {}
          };
        },
  ...
  describe('rechirp tests', function() {
    ...
    it('test multi-party exchange', function() {
      scope.chirpReply({ id: 2, user: { 'screen_name': 'mctee' }, text: '@currence.yusir @user1 @user2 please respond' });
    ...
    it('test regex for parsing user names', function() {
      scope.chirpReply({ id: 4, user: { 'screen_name': 'currence.yusir' }, text: '@user1 @user2 test@user3 please respond @user4' });
